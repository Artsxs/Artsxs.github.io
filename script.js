...
function girarComTodosOsPontos() {
  ...
  const quantidadeFinal = Math.min(quantidade, limiteSeguro);

  if (quantidade > limiteSeguro) {
    const confirmar = confirm(
      "Você tem pontos para " +
        formatarNumero(quantidade) +
        " giros. Para não travar o navegador, o jogo vai girar " +
        formatarNumero(limiteSeguro) +
        " vezes agora. Continuar?"
    );
  ...
  girarRng(quantidadeFinal, false);
}
...
function atualizarMochila() {
  ...
  const mochilaOrdenada = [...jogo.mochila].sort(function (a, b) {
    ...
    const aEquipado = jogo.equipados.includes(a.chave) ? 1 : 0;
    const bEquipado = jogo.equipados.includes(b.chave) ? 1 : 0;

    if (aEquipado !== bEquipado) {
      return bEquipado - aEquipado;
    }
...
function atualizarTela() {
  ...
  elementos.botaoRng100.disabled = jogo.pontos < precoRng * 100;
